...
/* 	结构体长这样：
	struct mmc_ioc_cmd {
		int write_flag;		//1表示读，0表示写
		int is_acmd;		//是不是ACMD(软件专属命令)
		__u32 opcode;		
		__u32 arg;
		__u32 response[4];
		unsigned int flags;
...
/* 	ioctl(int fd,int cmd,...)：设备控制接口函数，成功时返回0，失败返回-1；
	第一个参数是文件描述符；
	第二个参数是交互协议，理论上为整型，划分为四个位段；
	第三个是可变参数，依赖cmd指定长度和类型
	教程：https://blog.csdn.net/qq_19923217/article/details/82698787
	*/
...
int read_cid(int fd, __u8 *cid){
	...
	memset(&idata, 0, sizeof(idata));
	memset(cid, 0, sizeof(__u8) * 128);

	idata.write_flag = 0;
	idata.opcode = 10;
	idata.arg    = 0;
	idata.flags  = 
	MMC_RSP_SPI_R2 | MMC_RSP_R2 | MMC_CMD_ADTC;
...
int do_ffu(int nargs, char **argv)
{
	...
	// int i;
	// ret = read_cid(dev_fd, cid);
	// if (ret) {
	// 	fprintf(stderr, "Could not read CID from %s\n", device);
	// 	// goto out;
	// }else{
	// 	printf("read CID succcess!\n");
	...
	// 		OID=MID|(cid[i]<<(i-104));
	// 	}
	// 	if(MID!=0xDF||OID!=0x18){
	// 		fprintf(stderr,"not SCY!\n");
	// 		// goto out;
	// 	}
	// }
	...
	// MMC_SWITCH;
	multi_cmd->cmds[0].arg = 
	 0x031E0100;
	// (MMC_SWITCH_MODE_WRITE_BYTE << 24) |
	// 		(EXT_CSD_MODE_CONFIG << 16) |
	// 		(EXT_CSD_FFU_MODE << 8) |
	// 		EXT_CSD_CMD_SET_NORMAL;
	multi_cmd->cmds[0].flags = 
	//MMC_RSP_R1B | MMC_CMD_AC;
	MMC_RSP_SPI_R1B | MMC_RSP_R1B | MMC_CMD_AC;
	multi_cmd->cmds[0].write_flag = 1;

	/* send image chunk */
	multi_cmd->cmds[1].opcode = MMC_WRITE_MULTIPLE_BLOCK;
	...
	multi_cmd->cmds[2].arg = 
	 0x031E0000;
	// 		(MMC_SWITCH_MODE_WRITE_BYTE << 24) |
	// 		(EXT_CSD_MODE_CONFIG << 16) |
	// 		(EXT_CSD_NORMAL_MODE << 8) |
	// 		EXT_CSD_CMD_SET_NORMAL;
	multi_cmd->cmds[2].flags = 
		// MMC_RSP_R1B | MMC_CMD_AC;
	MMC_RSP_SPI_R1B | MMC_RSP_R1B | MMC_CMD_AC;
	multi_cmd->cmds[2].write_flag = 1;
	// printf("do_retry begin!\n");

	struct mmc_ioc_cmd cmd12;
	memset(&cmd12, 0, sizeof(cmd12));
	cmd12.opcode = 12;
	cmd12.arg = 0x0;
	cmd12.flags=
	MMC_RSP_SPI_R1B | MMC_RSP_R1B | MMC_CMD_AC;
	// cmd12.write_flag = 1;
	// ret = ioctl(dev_fd, MMC_IOC_CMD, &cmd12);
	// if(ret<0){
	// 	perror("cmd12_ioctl:");
	// }
...
// do_retry:
	...
	/* 读512个字符到buf里 524288*/
	ret = read(img_fd, buf, 512*1024);
	if(ret<0){
		perror("read fail!:");
	}

	printf("send cmd6 for ffu...\n");
	...
	// while (chunk_size > 0) {
		...
		// chunk_size = read(img_fd, buf, 512*256);
	// }
	printf("while cicle end!\n");
	...
	printf("send CMD6 ...\n");
	ret = ioctl(dev_fd, MMC_IOC_CMD, &multi_cmd->cmds[2]);
	if (ret) {
		perror("cmd6 to normal:");
	}else {
		printf("success！\n");
	}
	sleep(1);

	struct mmc_ioc_cmd cmd0;
	memset(&cmd0, 0, sizeof(cmd0));
	cmd0.opcode = 0;
	cmd0.arg = 0xFFFFFFFA;
	// cmd12.flags=0xFFFFFFFA;
	// MMC_RSP_SPI_R1B | MMC_RSP_R1B | MMC_CMD_AC;
	// cmd12.write_flag = 1;
	ret = ioctl(dev_fd, MMC_IOC_CMD, &cmd0);
	if(ret<0){
		perror("cmd0_ioctl:");
	}
