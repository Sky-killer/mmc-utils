...
int read_extcsd(int fd, __u8 *ext_csd)
{
	...
	ret = ioctl(fd, MMC_IOC_CMD, &idata);//============================================Connection timed out
	if (ret)
		perror("ioctl");
...
int do_read_extcsd(int nargs, char **argv)
{
	...
	printf("ext_csd_rev =%d",EXT_CSD_REV);
	switch (ext_csd_rev) {
	case 8:
		str = "5.1";
		break;
...
int do_FFU_1(int nargs, char **argv)
{
#ifndef MMC_IOC_MULTI_CMD
	fprintf(stderr, "mmc-utils has been compiled without MMC_IOC_MULTI_CMD"
			" support, needed by FFU.\n");
	...
	printf("do_read_cid:\n");
	ret=do_read_cid(nargs+1,argv);
	if (ret)
	{
		perror("do_read_cid failed\n");
	}else
	{
		printf("do_read_cid success\n");
	}
	...
	ret = read_extcsd(dev_fd, ext_csd);//=============================================不稳定的问题 mmcblk0 time connection out
	if (ret) {
		fprintf(stderr, "Could not read EXT_CSD from %s\n", device);
		goto out;
	}
	...
	/* put device into ffu mode */
	multi_cmd->cmds[0].opcode = MMC_SWITCH;
	multi_cmd->cmds[0].arg = 0x031E0100;
	...
	/* return device into normal mode */
	multi_cmd->cmds[2].opcode = MMC_SWITCH;
	multi_cmd->cmds[2].arg = 0x031E0000;
